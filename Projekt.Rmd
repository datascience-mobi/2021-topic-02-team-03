...
boxplot(t(data.new_sherk)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Upregulated genes in treatment with Sherk", ylab = "log2(gene expression)", xlab = "Upregulated genes", las = 2)
abline(h = c(6:12), col = "grey", lty = 3)
legend("topleft", c("Sherk1 after 3 days", "Sherk1 after 7 days"), col = c("red", "green"), pch = 15, bg = "yellow")
...
boxplot(t(data.new_erk12)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Upregulated genes in treatment with ERK1/2 inhibitor", ylab = "log2(gene expression)", xlab = "Upregulated genes", las = 2)
abline(h = c(6:14), col = "grey", lty = 3)
legend("topleft",c("ERK1/2 inhibitor after 3h", "ERK1/2 inhibitor after 24h"), col = c("red", "green"), pch = 15, bg = "yellow")
...
boxplot(t(data.new_trametinib)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Upregulated genes in treatment with Trametinib", ylab = "log2(gene expression)", xlab = "Upregulated genes", las = 2)
abline(h = c(6:14), col = "grey", lty = 3)
legend("left",c("Trametinib after 3h", "Trametinib after 24h"),col=c("red", "green"), pch = 15, bg = "yellow")
```
...
boxplot(t(data.new_sherk_downreg)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Downregulated genes in treatment with Sherk", ylab = "log2(gene expression)", xlab = "Downregulated genes", las = 2)
abline(h = c(6:12), col = "grey", lty = 3)
legend("topleft", c("Sherk1 after 3 days", "Sherk1 after 7 days"), col = c("red", "green"), pch = 15, bg = "yellow")
...
boxplot(t(data.new_erk12_downreg)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Downregulated genes in treatment with ERK1/2 inhibitor", ylab = "log2(gene expression)", xlab = "Downregulated genes", las = 2)
abline(h = c(6:14), col = "grey", lty = 3)
legend("topleft",c("ERK1/2 inhibitor after 3h", "ERK1/2 inhibitor after 24h"), col = c("red", "green"), pch = 15, bg = "yellow")
...
boxplot(t(data.new_trametinib_downreg)[,order.vector], cex.axis = 0.6, col = c("red", "green"), main = "Downregulated genes in treatment with Trametinib", ylab = "log2(gene expression)", xlab = "Downregulated genes", las = 2)
abline(h = c(6:14), col = "grey", lty = 3)
legend("topleft",c("Trametinib after 3h", "Trametinib after 24h"),col=c("red", "green"), pch = 15, bg = "yellow")
```
...
#from Binnur
# attention! : i gotta clean some stuff up, i did the same thing differently a couple of times, this is not the final version!!!

# my conclusion for all : the optimal number of clusters vary from test to test for all the gene expression data, k=2,3 seem to be the most common ones, however i cannot give any meaning to these clusters... via tests i found out that two clusters are the most "clustrable" ones, i don't know what these clusters could mean though
# my conclusion for sun : there is definetly a quality clustering with k=2, which is the sun and no sun data.

# ps: also did some hierarchical clustering to see the craziness and the accuracy
...
k.max <- 15
wss <- sapply(1:k.max, 
              function(k){kmeans(data_topVar, k, nstart=25,iter.max = 15 )$tot.withinss})
wss
...
##Elbow Method 2:
set.seed(1234)
fviz_nbclust(data_topVar, kmeans, method = "wss")
...
#visualizing the distance matrix (Euclidean distance)
distance <- get_dist(data_topVar)
fviz_dist(distance, gradient = list(low = "blue", mid = "white", high = "red"))

#Silhouette Method 1:
silh <- function(k) {
  km.res <- kmeans(data_topVar, centers = k, nstart = 25)
  ss <- silhouette(km.res$cluster, dist(data_topVar))
  mean(ss[, 3])
}

k.values <- 2:15
silh_values <- map_dbl(k.values, silh)

plot(k.values, silh_values, type = "b", pch = 19, frame = FALSE, 
       xlab = "Number of clusters K",
       ylab = "Average Silhouettes")

#Silhouette Method 2:
fviz_nbclust(data_topVar, kmeans, method = "silhouette")
...
# a third method i found on the internet
# https://uc-r.github.io/kmeans_clustering

set.seed(1234)
gap_stat <- clusGap(data_topVar, FUN = kmeans, nstart = 25,K.max = 10, B = 50)
...
# kmeans for everything

##k-means
km2 = kmeans(data_topVar,2,nstart = 25, iter.max = 15)
km3 = kmeans(data_topVar,3,nstart = 25,iter.max = 15)
km4 = kmeans(data_topVar,4, nstar = 25, iter.max = 15)

fviz_cluster(km2, data_topVar,palette = c("red", "blue"), geom = "point",ellipse.type = "convex", ggtheme = theme_bw(), title = "Clusters of TopVar Skin Spec Genes")

fviz_cluster(km3, data_topVar,palette = c("red", "blue", "green"), geom = "point",ellipse.type = "convex", ggtheme = theme_bw(), title = "Clusters of TopVar Skin Spec Genes")

fviz_cluster(km4, data_topVar, palette = c("red", "blue", "green", "purple"), geom = "point", ellipse.type = "convex",  ggtheme = theme_bw(), title = "Clusters of TopVar Skin Spec Genes")

# from richard's code, i left it here
###Plot graph for WSS
wss = sapply(2:10, function(k){kmeans(x = t(data_topVar), centers = k)$tot.withinss})
plot(2:10, wss, type = 'b', pch = 19, xlab = "Number of clusters", ylab ="WSS")

# this is to show the names on the graph
fviz_cluster(km3, data = data_topVar)
fviz_cluster(km4, data = data_topVar)

# this is to see which genes are in which cluster and which are the centers
km2$centers
km3$centers
km4$centers
km3$cluster
...
# kmeans for sun exposure data

# the same process all over again for sun exposure data:
both = rbind.data.frame(sun_exp, nosun_exp)

##Elbow Method 1:
set.seed(123) 
k.max <- 15
wsssun <- sapply(1:k.max, 
              function(k){kmeans(both, k, nstart=25,iter.max = 15 )$tot.withinss})
wsssun
plot(1:k.max, wsssun, type="b", pch = 19, frame = FALSE, 
     xlab="Number of clusters K",
     ylab="Total within-clusters sum of squares")
...
#Silhouette Method 1:
silh.sun <- function(k) {
  km.ss <- kmeans(both, centers = k, nstart = 25)
  ss <- silhouette(km.ss$cluster, dist(both))
  mean(ss[, 3])
}

k.values.sun <- 2:15
silh_values.sun <- map_dbl(k.values.sun, silh.sun)

plot(k.values.sun, silh_values.sun, type = "b", pch = 19, frame = FALSE, 
       xlab = "Number of clusters K",
       ylab = "Average Silhouettes")
...
# the third method i found on the internet: Gap statistic method
set.seed(123)
gap_stat.sun <- clusGap(both, FUN = kmeans, nstart = 25, K.max = 10, B = 50)
print(gap_stat.sun, method = "firstmax")
fviz_gap_stat(gap_stat.sun)
...
```{r}
##k-means
km2 = kmeans(both,2,nstart = 25,iter.max = 15)
km5 = kmeans(both,5,nstart = 25,iter.max = 15)

# k=2
fviz_cluster(km2, data = both, palette = c("red", "blue"), geom = "point", ellipse.type = "convex", ggtheme = theme_bw())
fviz_cluster(km2, data = both)

# k=5
fviz_cluster(km5, data = both, palette = c("red", "blue", "green", "purple", "orange"), geom = "point", ellipse.type = "convex", ggtheme = theme_bw())
fviz_cluster(km2, data = both)
```

```{r}

fviz_pca_ind(prcomp(data_topVar), title = "PCA - all data",
geom = "point", ggtheme = theme_classic())
```

```{r}
# K-means on the all dataset
km.res2 <- kmeans(data_topVar, 3)
fviz_cluster(list(data = data_topVar, cluster = km.res2$cluster),
ellipse.type = "convex", geom = "point", stand = FALSE,
palette = "jco", ggtheme = theme_classic())
# Hierarchical clustering on the all dataset
fviz_dend(hclust(dist(data_topVar)), k = 3, k_colors = "jco",
as.ggplot = TRUE, show_labels = FALSE)

# It can be seen that the k-means algorithm and the hierarchical clustering impose a classification on the all uniformly distributed data set even if there are no meaningful clusters present in it. This is why, clustering tendency assessment methods should be used to evaluate the validity of clustering analysis. That is, whether a given data set contains meaningful clusters.
# Source: Kassambara Alboukadel, 2017, Practical Guide to Cluster Analysis in R: Unsupervised Machine Learning, STHDA
```
```{r}
...
# A value of H about 0.5 means that the data D is uniformly distributed.
...
```{r}
set.seed(123)
hopkins(data_topVar, n = nrow(data_topVar)-1)

# we have clusterable data yay!
```
```{r}
# the visual assessment of clustering tendency: dissimilarity matrix.
fviz_dist(dist(data_topVar), show_labels = FALSE)+
labs(title = "all data")
```

```{r}
df = scale(data_topVar)
...
fviz_cluster(km.res, geom = "point", ellipse.type = "convex",
...
neg_sil_index <- which(sil[,3] < 0)
sil[neg_sil_index, , drop = FALSE]

# they are fairly good clusters
...
```{r}
km.res2 <- kmeans(data_topVar, 4)
fviz_cluster(list(data = data_topVar, cluster = km.res2$cluster),
ellipse.type = "convex", geom = "point", stand = FALSE,
palette = "jco", ggtheme = theme_classic())

km.res4 <- eclust(df, "kmeans", k = 4, nstart = 25, graph = FALSE)
fviz_silhouette(km.res4, palette = "jco",
ggtheme = theme_classic())
...
# four centers look better but the average silhouette width is too small (0.42), 3 clusters have a avsilwid of 0.46, and 2 clusters have 0.5. At this point i am so fcking confused as to choose which one
# i also dont have a clue what groups of genes the clusters are

```

```{r}
# same validation stuff and so for sun data:
dfsun = scale(both)

# Elbow method
fviz_nbclust(dfsun, kmeans, method = "wss") +
geom_vline(xintercept = 4, linetype = 2)+
labs(subtitle = "Elbow method")
# Silhouette method
fviz_nbclust(dfsun, kmeans, method = "silhouette")+
labs(subtitle = "Silhouette method")
# Gap statistic
set.seed(123)
fviz_nbclust(dfsun, kmeans, nstart = 25, method = "gap_stat", nboot = 50)+
labs(subtitle = "Gap statistic method")
nb <- NbClust(dfsun, distance = "euclidean", min.nc = 2,
max.nc = 10, method = "kmeans")
fviz_nbclust(nb)
km.res <- eclust(dfsun, "kmeans", k = 2, nstart = 25, graph = FALSE)
# Visualize k-means clusters
fviz_cluster(km.res, geom = "point", ellipse.type = "convex",
palette = "jco", ggtheme = theme_minimal())
# Hierarchical clustering
hc.res <- eclust(dfsun, "hclust", k = 2, hc_metric = "euclidean", hc_method = "ward.D2", graph = FALSE)
fviz_dend(hc.res, show_labels = FALSE,
palette = "jco", as.ggplot = TRUE)
# cluster validation: silhouette plot
fviz_silhouette(km.res, palette = "jco",
...
#this one is def two clusters

```

```{r}
# extracting rows didn't work for me for some reason so i went crazy, -> question for nils XD
...
```{r}
# same validation stuff and so for up/down:
...
max.nc = 10, method = "kmeans")
fviz_nbclust(nb)
km.res <- eclust(updown, "kmeans", k = 4, nstart = 25, graph = FALSE)
# Visualize k-means clusters
fviz_cluster(km.res, geom = "point", ellipse.type = "convex",
palette = "jco", ggtheme = theme_minimal())
# Hierarchical clustering
hc.res <- eclust(updown, "hclust", k = 4, hc_metric = "euclidean", hc_method = "ward.D2", graph = FALSE)
fviz_dend(hc.res, show_labels = FALSE,
palette = "jco", as.ggplot = TRUE)
# cluster validation: silhouette plot
fviz_silhouette(km.res, palette = "jco",
...
fviz_cluster(km.res, data = updown)

# some same genes are both upregulated and downregulated in diff treatments, this could be an explanation for the 4 clusters :)
