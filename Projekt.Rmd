...
In limma analysis we aimed to find the differential expressed genes (or DEGs). The idea behind limma is that it takes the normalized data and performs statistical tests to discover any quantitative changes in the gene expression, in our case over time. We decided to use limma as a model of comparison to the up regulated genes we have already found manually. It is to be seen from the results of limma analysis, that the genes were very similar to the ones found, which means that the manual way succeeded. This could be checked by doing the intersection of the vectors for the significant up regulated genes of each treatment and the top 10 genes found with limma.
This code was created with the help of the User Guide for limma from R.


```{r}
library(limma)
## First group: We compare the expression values from day 3 to day 7 with the shERK1 treatment
...
# EGLN3, DSP, PPP1R14C, SPINT2, MPZL3, GATA3, GAN, GJA1, DSC3, BCL11A
...
lola2 = lola2[1:10,]
# TRPM1, VANGL2, EFS, ETV7, ARHGEF5, PMEL, TYRP1, AACS, TGFBI, PPFIBP2

intersect(upreg_erk12_significant, rownames(lola2))
# "TRPM1"   "EFS"     "ETV7"    "ARHGEF5" "TYRP1"   "AACS"    "TGFBI"   "PPFIBP2"
...
lola3 = lola3[1:10,]
# VANGL2, TRPM1, ETV7, DAPL1, TGFBI, EFS, DLX5, DCT, TYRP1, ARHGEF5

intersect(upreg_trametinib_significant, rownames(lola3))
# "VANGL2" "TRPM1"  "ETV7"   "DAPL1"  "TGFBI"  "EFS"    "DCT"    "TYRP1" 
...
#Genes with the highest variance from PCA: 
...
```{r}
#Putting all upregulated genes in a data frame
...
```
```{r}
#compute correlation (to see the correlation value between two randomly picked genes)
...
```
Conclusion: according to the both methods there is a high correlation between all upregulated genes except MPZL3. For the linear regression only MPZL3 and one other gene representing all others will be used. 
...
#Visualization of the Pearson correlation
x <- cor(df_pca[1:10])
corrplot(x, type="upper", order="alphabet",title = "Correlation within the PCA genes-Spearman")

#Visualization of the Spearman correlation
y <- cor(df_pca[1:10], method = 'spearman')
corrplot(y, type="upper", order="alphabet", title = "Correlation within the PCA genes-Pearson")
```
```{r}
#compute correlation (to determine the correlation value between two randomly picked genes)
cor(df_pca$tgfbi,df_pca$anxa2) #high correlation
#test for significance
...
```
```{r}
#Another correlation visualization with scatterplots
#upregulated genes
pairs(df_upr,col='red',pch=20,cex=0.5)
#PCA genes
pairs(df_pca,col='orange',pch=20,cex=0.5)
```
Conclusion: Genes that have a high correlation will be considered as they have the same impact on the upregulated genes and a randomly picked gene among them will represent all others.
...
Since the p-value is lower than 0.01 we can reject the H0 and accept H1.
...
plot(all$mpzl3,mpzl3_lr$fitted.values,pch=20,col='blue', title("Prediction of MPZL3 expression"), xlab='Real values',ylab='Predicted values');abline(0,1,col='red')
...
#Plotting MPZL3 against only the variable that has the most impact -> PPP1R14C
...
#Simple plot of linear regression model
...
plot(all$aacs, aacs_lr$fitted.values,pch=20,col='blue', title("Prediction of the AACS expression"), xlab='Real values',ylab='Predicted values');abline(0,1,col='red')
