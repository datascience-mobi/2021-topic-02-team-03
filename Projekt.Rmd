...
colnames(data.skin.cancer.human_skin_TRA)
head(data.skin.cancer.human_skin_TRA)
```
...
par(las=2)
boxplot(t(data.skin.cancer.human_skin_TRA),col=rainbow(length(rownames(data.skin.cancer.human_skin_TRA))),main="all genes",cex.axis=0.8)

# putting the upregulated genes in a vector, i chose 12 after looking at the overall distribution of our gene expression above, we can change it later if you guys want to
data <- data.skin.cancer.human_skin_TRA
upregulated <- data[which(data[,1]>12),]
# note that this is only for chip one, i have to work on how to look at all of the chips' expressions at the same time

# looking at the expressions of the upregulated genes
par(las=2)
boxplot(t(upregulated),col=rainbow(length(rownames(upregulated))),main="upregulated",cex.axis=0.8)

# specifically the highly upregulated ones, the ones we'll be dealing with
mostupregulated <- data[which(data[,1]>13.5),]
boxplot(t(mostupregulated),col=rainbow(length(rownames(mostupregulated))),main="most upregulated",cex.axis=0.8)
# i have noted the expressions of these for every chip, and they vary except for two highest: GPNMB and ANXA2
# except for these two NDRG1, RPL18, DCT, CD44, MLANA also showed high expressions, although varying between chips, the decision of if we're going to involve these too or not would only make sense after comparing the expression data of these genes over all the chips at the same time, i'm open to ideas and help on that XD
```

# Sun exposure, different times, and different drugs

```{r}

# boxplots of sun exposure vs no sun exposure in all data

#finding the sun exposed and sun not exposed genes in our expression dataframe
...
#removing the gene names, so that i can plot the gene expression only from one chip
...
par(mfrow = c(1, 2))
boxplot(sunexpchip,ylim=lmts)
boxplot(nosunexpchip,ylim=lmts)

#i tried different chips as i put their numbers in the code above one by one, and they don't seem to differ significantly so i just left it at that.
#however i did try what Maria showed in github for different times, and different drugs:

# boxplots of upregulated expressions with different hours 
...
data.fortime=rbind(mostupregulated,mostupregulated,mostupregulated,mostupregulated)
data.fortime[c(6:9),c(three.days,seven.days,twentyfour.hours)]=NA
data.fortime[c(10:15),c(seven.days,three.days,three.hours)]=NA
data.fortime[c(1:3),c(twentyfour.hours,three.hours,seven.days)]=NA
data.fortime[c(4:5),c(twentyfour.hours,three.hours,three.days)]=NA

order.vector=sort(colnames(t(data.new)),index.return=T)$ix
boxplot(t(data.new)[,order.vector],cex.axis=0.6,col=c("pink","red","yellow", "purple"),main="skin specific TRA expression in skin cancer",ylab="log2(gene expression)")
abline(h=c(6:12),col="grey",lty=3)
legend("topright",c("3h","24h","3d", "7d"),col=c("pink","red","yellow", "purple"),pch=15,bg="white")

# boxplots of upregulated expressions with different drugs

sherk1 <- data[,1:5]
ERK12inh <- c(data[,6:7], data[10:12])
trametinib <- c(data[8:9], data[13:15])

data.fordrug=rbind(mostupregulated,mostupregulated,mostupregulated)
...
# Edited version of the second half of the above chunk on 01.07.2021 + some stuff
...
a.data.fortime=rbind(mostupregulated,mostupregulated,mostupregulated,mostupregulated)
a.data.fortime[c(1:4),c(4:15)]=NA
a.data.fortime[c(5:8),c(1:3,6:15)]=NA
a.data.fortime[c(9:12),c(1:5,10:15)]=NA
a.data.fortime[c(13:16),c(1:9)]=NA

a.order.vector=sort(colnames(t(a.data.fortime)),index.return=T)$ix
boxplot(t(a.data.fortime)[,a.order.vector],cex.axis=0.6,col=c("pink","red","yellow", "purple"),main="skin specific TRA expression in skin cancer",ylab="log2(gene expression)")
abline(h=c(6:12),col="grey",lty=3)
legend("topright",c("3h","24h","3d", "7d"),col=c("pink","red","yellow", "purple"),pch=15,bg="white")
...
a.data.new[c(16:20), c(1:7, 10:15)]=NA
a.data.new[c(21:25), c(1:9, 13:15)]=NA
...
abline(h=c(6:12),col="grey",lty=3)
legend("bottomleft",c("sherk1 after 3 days", "sherk1 after 7 days", "ERK12inh after 3h", "trametinib after 3h", "ERK12inh after 24h", "trametinib after 24h"),col=c("pink","red", "green","yellow","blue","purple"),pch=15,bg="white")


Problem: this is only for most upregulated genes in CHIP 1, this has to be repeated for all OTHER chips (so also different number of genes) --> need for loop with length(mostupregulated)!!!!


median(data)

mostupregulated <- data[which(data[,1]>3.75),]
a.data.new=rbind(mostupregulated,mostupregulated,mostupregulated,mostupregulated,mostupregulated,mostupregulated)
a.data.new[c(1:4), c(4:15)]=NA
a.data.new[c(5:8), c(1:3, 6:15)]=NA
a.data.new[c(9:12), c(1:5, 8:15)]=NA
a.data.new[c(13:16), c(1:7, 10:15)]=NA
a.data.new[c(17:20), c(1:9, 13:15)]=NA
...
a.km = kmeans(x = data[rownames(upregulated),], centers = 4, nstart = 15)
table(a.km$cluster)
a.km
...
k = c()
for(i in 1:15){
  k = c(k, rownames(data[which(data[,i]>3.5),]))
}
k

highly_expressed = unique(k)
variable_expressed = highly_expressed[c(26, 75, 86, 87, 103)]
```
