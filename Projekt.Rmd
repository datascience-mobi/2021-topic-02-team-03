...
setwd(paste(projectPath, 'plots', sep = '/'))
dev.copy2eps(file = 'Heatmap for skin specific TRAs in skin cancer.eps') #eps file
dev.copy2pdf(file = 'Heatmap for skin specific TRAs in skin cancer.pdf') #pdf file
```
...
lmts <- range(sunexpchip1,nosunexpchip1)
...
data.fortimep=rbind(mostupregulated,mostupregulated,mostupregulated,mostupregulated)
...
data.new=rbind(mostupregulated,mostupregulated,mostupregulated)
data.new[c(1:5),c(ERK12inh, trametinib)]=NA
data.new[c(6:7, 10:12),c(sherk1,trametinib)]=NA
data.new[c(8:9, 13:15),c(sherk1, ERK12inh)]=NA

order.vector=sort(colnames(t(data.new)),index.return=T)$ix
boxplot(t(data.new)[,order.vector],cex.axis=0.6,col=c("pink","red", "purple"),main="diff drugs",ylab="log2(gene expression)")
abline(h=c(6:12),col="grey",lty=3)
legend("topright",c("sherk1","ERK12inh","trametinib"),col=c("pink","red","purple"),pch=15,bg="white")

# either i'm doing something wrong here, or there is really no significant difference between the data of sun or not, different drugs, and different times, which i think makes sense. still, please let me know your thoughts.
