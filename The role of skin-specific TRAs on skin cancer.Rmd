...
#Genes that have a high correlation will be considered as they have the same impact on the upregulated genes. DCT, GJA1 and TGFBI will be used for the linear regression (5 genes from PCA used in total).
```
...
mpzl3_lr = lm(MPZL3 ~ DCT+GJA1+TGFBI, data = data.frame(t(data.skin.cancer.human_skin_TRA)))
summary(mpzl3_lr)
#We can see that there is a significance on GJA1. The p-value is also very small (<0.01), meaning that this model is significantly better than the null model. R-squared value is also high, meaning it was a good model (proportion of variance explained is high).
...
plot(data.skin.cancer.human_skin_TRA["GJA1",],mpzl3_lr$residuals,pch=20, title("Correlation of the residuals"), xlab="MPZL3", ylab="MPZL3_lr-Residuals")
...
lm.beta(mpzl3_lr)
#This means a change in 1 standard deviation of GJA1 has more impact than a 1 standard deviation change in other variables. 

#Plotting MPZL3 against only the variable that has the most impact (GJA1)
mpzl3_gja1 = lm(MPZL3 ~ GJA1, data = data.frame(t(data.skin.cancer.human_skin_TRA)))
summary(mpzl3_gja1)

df_mpzl3_gja1 = data.frame(y= data.skin.cancer.human_skin_TRA["MPZL3",], x= data.skin.cancer.human_skin_TRA["GJA1",])

plot(df_mpzl3_gja1$x, df_mpzl3_ppp1r14c$y, xlab="Predictor(GJA1)", ylab="Outcome(MPZL3)", main="Simple linear regression of MPZL3 by GJA1", col="darkorange",pch=16)
abline(mpzl3_gja1, col="red")
for (i in 1:length(mpzl3_gja1$fitted.values))
  lines(c(df_mpzl3_gja1$x[i], df_mpzl3_gja1$x[i]), c(df_mpzl3_gja1$y[i], mpzl3_gja1$fitted.values[i]), col="blue")
```
