...
Downregulated genes show that the drug has worked on genes that were tumour inducing at the beginning. Analysis of these genes was made to see if there are any interesting genes that may have a signficant meaning to the treatment.
...
to show melanoma is caused by uv/sun --> which upregulated genes are sun exposed genes (related to uv exposure)
...
fviz_cluster(km.both, data = both, palette = c("orange", "blue"), geom = "point", main = "Clustering of genes with or without sun exposure") + theme(plot.title = element_text(hjust = 0.5))
...
## 4.2. Clustering of the upregulated skin specific TRAs
...
#Is the data really clusterable?
hopkins(upregulated_all_exp, n = nrow(upregulated_all_exp)-1)#The H value is also lower than 0.5 and is relatively close to 0, so data probably has clusters.
...
top_genes_pca2 = names(sort(abs(skin_pca_topVar$rotation[,2]), decreasing = T)[1:6])
...
## 6.1. Correlation between genes

Correlation between upregulated genes and genes obtained from PCA must first be done
...
#From the plot it can be seen that there is a high correlation between all upregulated genes except MPZL3. For the linear regression only MPZL3 and one other gene representing all others will be used (2 upregulated genes used in total).
...
#Genes that have a high correlation will be considered as they have the same impact on the upregulated genes. DCT, DSP and TGFBI will be used for the linear regression (5 genes from PCA used in total).
```

## 6.2. Linear regression and F-Tes

F-Test:
H0: The model doesn't refer to any significance.
H1: The model refers to a significance.
If the p-value is lower than the chosen threshold (in our case we chose 0.01), it is proven that the independent variables improve the fit in our model

The following have to be checked:
* the residuals are **normally distributed**
* there is **no correlation** between the residuals and the explanatory variable
* mean values of the residuals are close to 0

## 6.2.1. Multiple linear regression of MPZL3

```{r}
#MPZL3 expression predicted with PCA genes
mpzl3_lr = lm(MPZL3 ~ DCT+DSP+TGFBI, data = data.frame(t(data.skin.cancer.human_skin_TRA)))
summary(mpzl3_lr)
#We can see that there is a significance on DSP. The p-value is also very small (<0.01), meaning that this model is significantly better than the null model. R-squared value is also high, meaning it was a good model (proportion of variance explained is high).

#Normal distribution of residuals
hist(mpzl3_lr$residuals,breaks=20)
shapiro.test(mpzl3_lr$residuals) #P-value is very high, meaning its almost definitely normally distributed.
...
cor(data.skin.cancer.human_skin_TRA["DCT",], mpzl3_lr$residuals)
cor(data.skin.cancer.human_skin_TRA["DSP",], mpzl3_lr$residuals)
cor(data.skin.cancer.human_skin_TRA["TGFBI",], mpzl3_lr$residuals)
#No correlation between any of the explanatory genes and the residuals.

#Mean of the residuals
mean(mpzl3_lr$residuals) #Mean is almost 0.
...
#We can see that the predicted values and the real values are quite similar, indicating that these genes have a strong relationship. If one is being highly expressed the other one would also be so.
```
...
## 6.2.3. Multiple linear regression of ARHGEF5

```{r}
#ARHGEF5 expression predicted with PCA genes
arhgef5_lr = lm(ARHGEF5 ~ DCT+GJA1+TGFBI, data = data.frame(t(data.skin.cancer.human_skin_TRA)))
summary(arhgef5_lr)
#We can see that there is a significance on DCT, but especially on TGFBI. The p-value is also very small (<0.01), meaning that this model is significantly better than the null model. R-squared value is also high, meaning it was a good model (proportion of variance explained is high).

#Normal distribution of residuals
hist(arhgef5_lr$residuals,breaks=20)
shapiro.test(arhgef5_lr$residuals) #P-value is relatively high, meaning its probably normally distributed.

#Correlation within residuals
plot(data.skin.cancer.human_skin_TRA["TGFBI",],arhgef5_lr$residuals,pch=20, title("Correlation of the residuals"), xlab="ARHGEF5", ylab="ARHGEF5_lr-Residuals")
cor(data.skin.cancer.human_skin_TRA["DCT",], arhgef5_lr$residuals)
cor(data.skin.cancer.human_skin_TRA["GJA1",], arhgef5_lr$residuals)
cor(data.skin.cancer.human_skin_TRA["TGFBI",], arhgef5_lr$residuals)
#No correlation between any of the explanatory genes and the residuals.

#Mean of the residuals
mean(arhgef5_lr$residuals) #Mean is almost 0.
...
#Here the predicted values and the real values are very similar everywhere along the regression line, meaning these genes have a very strong relationship. The expressions of ARHGEF5 and TGFBI are directly proportional to each other.
```

## 6.2.2. Simple linear regression of ARHGEF5
...
arhgef5_tgfb1 = lm(ARHGEF5 ~ TGFBI, data = data.frame(t(data.skin.cancer.human_skin_TRA)))
summary(arhgef5_tgfb1)

df_arhgef5_tgfb1 = data.frame(y= data.skin.cancer.human_skin_TRA["ARHGEF5",], x= data.skin.cancer.human_skin_TRA["TGFBI",])

plot(df_arhgef5_tgfb1$x, df_arhgef5_tgfb1$y, xlab="Predictor(TGFBI)", ylab="Outcome(ARHGEF5)", main="Simple linear regression of ARHGEF5 by TGFBI", col="darkorange",pch=16)
abline(arhgef5_tgfb1, col="red")
for (i in 1:length(arhgef5_tgfb1$fitted.values))
  lines(c(df_arhgef5_tgfb1$x[i], df_arhgef5_tgfb1$x[i]), c(df_arhgef5_tgfb1$y[i], arhgef5_tgfb1$fitted.values[i]), col="blue")

#The actual reason I picked ARHGEF5 is because apparently ARHGEF5 is downstream of TGFBI (they are on the same pathway), so this would make sense that they are connected to each other. More info abt this will be sent by denisa, bcos she searched for the genes for clustering and found this. Very good to be put in the discussion!
```
